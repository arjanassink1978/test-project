...
describe("ForumCategoryFilter", () => {
  ...
  it("applies primary className to All button when selectedId is null", () => {
    ...
    expect(allBtn.className).toBe(button.primary);
  });
  ...
  it("applies secondary className to All button when a category is selected", () => {
    ...
    const allBtn = screen.getByTestId("category-option-all");
    expect(allBtn.className).toBe(button.secondary);
  });
  ...
  it("applies primary className to selected category button", () => {
    ...
    expect(cat2Btn.className).toBe(button.primary);
  });
  ...
  it("applies secondary className to non-selected category buttons", () => {
    ...
    expect(cat1Btn.className).toBe(button.secondary);
    expect(cat3Btn.className).toBe(button.secondary);
  });
  ...
  it("when selectedId switches, className updates correctly", () => {
    ...
    expect(screen.getByTestId("category-option-1").className).toBe(button.primary);
    expect(screen.getByTestId("category-option-2").className).toBe(button.secondary);

    rerender(<ForumCategoryFilter categories={categories} selectedId={2} onChange={jest.fn()} />);
    expect(screen.getByTestId("category-option-1").className).toBe(button.secondary);
    expect(screen.getByTestId("category-option-2").className).toBe(button.primary);
  });

  it("when selectedId is null, All has primary and all categories have secondary", () => {
    render(<ForumCategoryFilter categories={categories} selectedId={null} onChange={jest.fn()} />);
    expect(screen.getByTestId("category-option-all").className).toBe(button.primary);
    categories.forEach((cat) => {
      expect(screen.getByTestId(`category-option-${cat.id}`).className).toBe(button.secondary);
    });
  });
